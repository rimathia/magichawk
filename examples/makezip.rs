...
fn main() {
    ...
    let mut f = std::io::Cursor::new(&mut outputbuffer);
    ...
    let zipfile = File::create("example.zip").unwrap();
    let mut zip = zip::ZipWriter::new(zipfile);
    zip.start_file("example.tiff", zip::write::FileOptions::default());
    zip.write(&outputbuffer);
    zip.finish().unwrap();
}
